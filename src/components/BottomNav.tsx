import { Home, AlertTriangle, MessageSquare, BarChart3, Settings, BookOpen, Hash } from "lucide-react";
...
const BottomNav = ({ activeTab, onTabChange, alertCount = 3 }: BottomNavProps) => {
  const tabs = [
    ...
    { id: "narratives" as Tab, icon: Hash, label: "Narrativas" },
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center flex-1 h-full relative transition-colors ${isActive ? 'text-primary' : 'text-muted-foreground'
                }`}
            >
              <div className="relative">
                <tab.icon className={`w-5 h-5 mb-1 transition-transform ${isActive ? 'scale-110' : ''
                  }`} />
